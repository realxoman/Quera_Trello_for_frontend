...
from workspace.api.serializers.serializer_task_assignee import (
...
class TaskAssigneeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return TaskAssignee.objects.filter(task_id=self.kwargs["task_id"])

    def get_serializer_class(self):
        if self.action == "create":
            return TaskAssigneeCreateSerializer
